...
function getFooterTemplate(path) {
    path = "https://github.com/oi-contrib/VISLite/edit/v0.x/" + path.replace('pages/example', 'examples');
...
function getStyle(el, name) {
    var allStyle = document.defaultView && document.defaultView.getComputedStyle ? document.defaultView.getComputedStyle(el, null) : el.currentStyle;
    return allStyle.getPropertyValue(name);
};

// 使得一个结点可以拖拽
function bindDragMove(el) {
    var isDown = false;
    ...
    el.addEventListener("mousedown", function (event) {
        ...
        left = getStyle(el, 'left').replace('px', '');
        top = getStyle(el, 'top').replace('px', '');
